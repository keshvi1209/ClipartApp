...
export default function ResultScreen() {
  ...
  const selectedStyleIds = selectedStyles
    ? (selectedStyles.split(",") as StyleId[])
    : [];
  ...
  const handleDownloadAll = async () => {
    ...
    for (const r of successful) {
      try {
        const fileUri =
          FileSystem.cacheDirectory + `clipart_${r.styleId}_${Date.now()}.png`;
      ...
      <View style={styles.header}>
        ...
        {isDone && (
          <TouchableOpacity onPress={handleRegenerate} style={styles.regenBtn}>
            <Feather name="refresh-cw" size={18} color={colors.mint} />
          </TouchableOpacity>
        )}
      </View>
            ...
            <View style={styles.resultsGrid}>
              {results.map((result) => (
                <ResultCard
                  ...
                  result={result}
                  onRetry={handleRetry}
                  onExpand={(url, styleId) =>
                    setExpandedImage({ url, styleId })
                  }
                />
              ))}
            </View>
            ...
            <>
              ...
              <View style={styles.modalActions}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() =>
                    shareImage(expandedImage.url, expandedImage.styleId)
                  }
                >
